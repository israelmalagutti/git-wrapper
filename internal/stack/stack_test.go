...
// MockRepo implements a minimal git.Repo interface for testing
...
func (m *MockRepo) BranchExists(name string) bool {
	return m.branches[name]
}

func (m *MockRepo) GetBranchCommit(name string) (string, error) {
	return m.commits[name], nil
}

func newMockRepo() *MockRepo {
	return &MockRepo{
		currentBranch: "feat-1",
		branches: map[string]bool{
			"main":   true,
			"feat-1": true,
			"feat-2": true,
...
func TestValidateStack(t *testing.T) {
	...
	t.Run("detects cycle", func(t *testing.T) {
		...
		feat2 := &Node{Name: "feat-2", Parent: feat1}
		feat1.Parent = feat2 // Create cycle
